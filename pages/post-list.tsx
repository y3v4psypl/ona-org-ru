...
export default function PostList() {
    ...
    useEffect(() => {
        ...
        postList.then(res => {
            setPostListItems(res.map((post, i) => <li key={i}>🚺 {post.title} 🚺<br/>{post.description.length + `🚺 ${post.title} 🚺`.length + post.link.length <= 280 ? post.description : post.description.slice(0, post.description.lastIndexOf('. '))}<br/>{post.link}</li>))
            setState('success')
        })
